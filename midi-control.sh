...
# KEYBOARD CONFIG
################################
#KEYBOARD_DEVICE="/dev/input/by-id/usb-Adafruit_ocreeb12-event-kbd"
KEYBOARD_DEVICE="/dev/input/by-id/usb-Adafruit_ocreeb12_vial:f64c2b3c-event-kbd"

KEY_AUX_SWITCH="KEY_F21"
KEY_MIC_MUTE="KEY_F19"
KEY_SPK_MUTE="KEY_F18"
KEY_HP_MUTE="KEY_F17"
...
action_prev() { playerctl previous; }
action_play() { playerctl play-pause; }
action_stop() { playerctl stop; }
action_next() { playerctl next; }

action_aux_switch() {
  [ "$AUX_TARGET" = "headphones" ] && aux_to_speakers || aux_to_headphones
}

action_mic_mute() { toggle_mic_mute; }
action_spk_mute() { toggle_spk_mute; }
action_hp_mute()  { toggle_hp_mute; }

################################
# INPUT LOOP
################################

if [ "$INPUT_MODE" = "MIDI" ]; then

  echo "🎹 Using MIDI input on $MIDI_PORT"
  ...
  aseqdump -p "$MIDI_PORT" | while read -r line; do
    ...
    # MEDIA
    ################################
    grep -q "Note on.* note $NOTE_PREV," <<<"$line" && action_prev
    grep -q "Note on.* note $NOTE_PLAY," <<<"$line" && action_play
    grep -q "Note on.* note $NOTE_STOP," <<<"$line" && action_stop
    grep -q "Note on.* note $NOTE_NEXT," <<<"$line" && action_next
    ...
    # MUTES
    ################################
    grep -q "Note on.* note $NOTE_HP_MUTE," <<<"$line" && action_hp_mute
    grep -q "Note on.* note $NOTE_SPK_MUTE," <<<"$line" && action_spk_mute
    grep -q "Note on.* note $NOTE_MIC_MUTE," <<<"$line" && action_mic_mute

  done

else

  echo "⌨️ Using Keyboard input on $KEYBOARD_DEVICE"

  # evtest --grab "$KEYBOARD_DEVICE" 2>/dev/null | while read -r line; do
  evtest "$KEYBOARD_DEVICE" 2>/dev/null | while read -r line; do

  [[ "$line" =~ value\ 1 ]] || continue

  grep -q "$KEY_AUX_SWITCH" <<<"$line" && action_aux_switch
  grep -q "$KEY_MIC_MUTE"   <<<"$line" && action_mic_mute
  grep -q "$KEY_SPK_MUTE"   <<<"$line" && action_spk_mute
  grep -q "$KEY_HP_MUTE"    <<<"$line" && action_hp_mute
